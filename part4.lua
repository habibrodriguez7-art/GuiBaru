-- LynxGUI_v2.3.lua - Modern Redesign Edition
-- FREE NOT FOR SALE
...
repeat wait(0.1) until localPlayer:FindFirstChild("PlayerGui")
...
-- SECURITY LOADER
-- ============================================
local success, func = pcall(loadstring, game:HttpGet("https://raw.githubusercontent.com/habibrodriguez7-art/GuiBaru/refs/heads/main/SecurityLoader.lua"))
if not success or not func then
    warn("Failed to load SecurityLoader script: " .. tostring(func))
    return
end

local success2, SecurityLoader = pcall(func)
if not success2 or not SecurityLoader then
    warn("Failed to execute SecurityLoader: " .. tostring(SecurityLoader))
    return
end

local success3, CombinedModules = pcall(SecurityLoader.LoadModule, "CombinedModules")
if not success3 or not CombinedModules then
    warn("Failed to load CombinedModules: " .. tostring(CombinedModules))
    return
end
...
local colors = {
    ...
    secondary = Color3.fromRGB(99, 179, 237),
    accent = Color3.fromRGB(168, 85, 247),
    success = Color3.fromRGB(16, 185, 129),
    
    bg1 = Color3.fromRGB(13, 17, 23),
    bg2 = Color3.fromRGB(22, 27, 34),
    bg3 = Color3.fromRGB(33, 38, 45),
    bg4 = Color3.fromRGB(48, 54, 61),
    
    text = Color3.fromRGB(230, 237, 243),
    textDim = Color3.fromRGB(139, 148, 158),
    textDimmer = Color3.fromRGB(110, 118, 129),
...
-- ============================================
-- PNG ICON ASSETS (Replace with your Roblox Asset IDs)
-- ============================================
local IconAssets = {
    Home = "rbxassetid://10734950309",        -- Dashboard icon
    Teleport = "rbxassetid://10734949856",    -- Location icon
    Shop = "rbxassetid://10747372992",        -- Shopping bag icon
    Webhook = "rbxassetid://10747384394",     -- Link/webhook icon
    Camera = "rbxassetid://10734898355",      -- Camera icon
    Settings = "rbxassetid://10734950309",    -- Gear icon
    Info = "rbxassetid://10734921959",        -- Info/help icon
}
...
new("UICorner",{Parent=versionBadge, CornerRadius=UDim.new(0, 8)})
...
local topBarStroke = new("UIStroke",{
    Parent=topBar,
    Color=colors.border,
    Thickness=1,
    Transparency=0.8
...
-- ============================================
-- NAV BUTTON WITH PNG ICONS
-- ============================================
local function createNavButton(text, iconAssetId, page, order)
...
local function switchPage(pageName, pageTitle_text)
    ...
    for name, btnData in pairs(navButtons) do
        ...
        if USE_TWEEN then
            ...
            TweenService:Create(btnData.stroke, TweenInfo.new(TWEEN_SPEED), {
                Color=isActive and colors.primary or colors.border,
                Transparency=isActive and 0.5 or 0.8
            }):Play()
            ...
            btnData.btn.BackgroundColor3 = isActive and colors.bg3 or colors.bg2
            btnData.stroke.Color = isActive and colors.primary or colors.border
            btnData.stroke.Transparency = isActive and 0.5 or 0.8
...
-- Create navigation buttons with PNG icons
local btnMain = createNavButton("Dashboard", IconAssets.Home, "Main", 1)
local btnTeleport = createNavButton("Teleport", IconAssets.Teleport, "Teleport", 2)
local btnShop = createNavButton("Shop", IconAssets.Shop, "Shop", 3)
local btnWebhook = createNavButton("Webhook", IconAssets.Webhook, "Webhook", 4)
local btnCameraView = createNavButton("Camera View", IconAssets.Camera, "CameraView", 5)
local btnSettings = createNavButton("Settings", IconAssets.Settings, "Settings", 6)
local btnInfo = createNavButton("About", IconAssets.Info, "Info", 7)
...
btnWebhook.MouseButton1Click:Connect(function() switchPage("Webhook", "Webhook Configuration") end)
btnCameraView.MouseButton1Click:Connect(function() switchPage("CameraView", "Camera Controls") end)
btnSettings.MouseButton1Click:Connect(function() switchPage("Settings", "Settings") end)
btnInfo.MouseButton1Click:Connect(function() switchPage("Info", "About Lynx") end)

-- ============================================
-- UI COMPONENTS - MODERN REDESIGN
-- ============================================

-- CATEGORY - COMPACT & CLEAN
local function makeCategory(parent, title, icon)
    ...
    header.MouseButton1Click:Connect(function()
        ...
        if USE_TWEEN then
            ...
            TweenService:Create(catStroke, TweenInfo.new(TWEEN_SPEED), {
                Transparency=isOpen and 0.5 or 0.7
            }):Play()
        else
            arrow.Rotation = isOpen and 180 or 0
            catStroke.Transparency = isOpen and 0.5 or 0.7
        end
    end)
...
local function makeToggle(parent, label, param3, param4)
    ...
    toggleCircle.Position = on and UDim2.new(1, -19, 0.5, -8) or UDim2.new(0, 3, 0.5, -8)
...
local function makeDropdown(parent, title, icon, items, param5, param6, param7)
    ...
    header.MouseButton1Click:Connect(function()
        ...
        if USE_TWEEN then
            ...
            TweenService:Create(dropStroke, TweenInfo.new(TWEEN_SPEED), {
                Transparency=isOpen and 0.4 or 0.6
            }):Play()
        else
            arrow.Rotation = isOpen and 180 or 0
            dropStroke.Transparency = isOpen and 0.4 or 0.6
        end
        
        if isOpen then
            listContainer.Size = UDim2.new(1, -16, 0, math.min(#items * 30, 140))
        end
    end)
    ...
    for _, itemName in ipairs(items) do
        ...
        itemBtn.MouseButton1Click:Connect(function()
            ...
            if USE_TWEEN then
                ...
                TweenService:Create(dropStroke, TweenInfo.new(TWEEN_SPEED), {Transparency=0.6}):Play()
            else
                arrow.Rotation = 0
...
local function makeButton(parent, label, callback)
    ...
    button.MouseButton1Click:Connect(function()
        if USE_TWEEN then
            TweenService:Create(btnFrame, TweenInfo.new(0.05), {Size=UDim2.new(0.98, 0, 0, 32)}):Play()
            task.wait(0.05)
            TweenService:Create(btnFrame, TweenInfo.new(0.05), {Size=UDim2.new(1, 0, 0, 34)}):Play()
        end
        pcall(callback)
    end)
...
-- ============================================
-- MULTI-SELECT DROPDOWN - MODERN COMPACT
-- ============================================
local function makeMultiSelectDropdown(parent, label, options, callback, configPath)
    ...
    for _, option in ipairs(options) do
        ...
        local optBtnStroke = new("UIStroke",{
            Parent=optionButton,
            Color=colors.border,
            Thickness=1,
            Transparency=0.7
        ...
        if selectedItems[option] then
            ...
            checkStroke.Color = colors.primary
            optionButton.BackgroundColor3 = colors.bg4
            optBtnStroke.Transparency = 0.5
        end
        ...
        optionButton.MouseButton1Click:Connect(function()
            if selectedItems[option] then
                ...
                if USE_TWEEN then
                    ...
                    TweenService:Create(checkStroke, TweenInfo.new(TWEEN_SPEED), {
                        Color=colors.border
                    }):Play()
                    TweenService:Create(optionButton, TweenInfo.new(TWEEN_SPEED), {
                        BackgroundColor3=colors.bg3
                    }):Play()
                    TweenService:Create(optBtnStroke, TweenInfo.new(TWEEN_SPEED), {
                        Transparency=0.7
                    }):Play()
                else
                    checkbox.BackgroundColor3 = colors.bg1
                    checkStroke.Color = colors.border
                    optionButton.BackgroundColor3 = colors.bg3
    ...
    dropdownButton.MouseButton1Click:Connect(function()
        ...
        if isOpen then
            local maxHeight = math.min(150, #options * 33 + 12)
            optionsContainer.Size = UDim2.new(1, -20, 0, maxHeight)
        else
            ...
            for _, child in ipairs(optionsContainer:GetChildren()) do
                if child:IsA("TextButton") then
                    ...
                    if checkbox then
                        ...
                        checkbox.BackgroundColor3 = colors.bg1
                        local checkStroke = checkbox:FindFirstChildOfClass("UIStroke")
                        if checkStroke then checkStroke.Color = colors.border end
                    end
                    child.BackgroundColor3 = colors.bg3
                    local btnStroke = child:FindFirstChildOfClass("UIStroke")
                    if btnStroke then btnStroke.Transparency = 0.7 end
                end
            end
            callback({})
        end
...
local function makeTextBox(parent, label, placeholder, defaultValue, callback)
    ...
    local textBoxStroke = new("UIStroke", {
        Parent = textBox,
        Color = colors.border,
        Thickness = 1,
        Transparency = 0.6
    ...
    textBox.Focused:Connect(function()
        if USE_TWEEN then
            TweenService:Create(textBoxStroke, TweenInfo.new(TWEEN_SPEED), {
                Color = colors.primary,
                Transparency = 0.3
            }):Play()
        else
...
-- ============================================
-- MAIN PAGE FEATURES - AUTO FISHING
-- ============================================
...
-- ============================================
-- TELEPORT PAGE - LOCATION TELEPORT
-- ============================================
local locationItems = {}
...
-- ============================================
-- CAMERA VIEW PAGE - UNLIMITED ZOOM
-- ============================================
...
-- ============================================
-- INFO PAGE - MODERN ABOUT SECTION
-- ============================================
...
new("UICorner",{Parent=infoHeader, CornerRadius=UDim.new(0, 8)})
...
local headerSubtitle = new("TextLabel",{
    Parent=infoHeader,
    Text="Free Edition • Refined 2024",
...
-- Content Section
local infoContent = new("ScrollingFrame",{
    Parent=infoContainer,
...
-- Feature Info Box
local function createInfoBox(title, description, icon, order)
    local box = new("Frame",{
        Parent=infoContent,
...
createInfoBox("Auto Fishing", "Advanced fishing automation with multiple modes", "🎣", 1)
createInfoBox("Teleportation", "Instant teleport to locations, players, and events", "🌍", 2)
createInfoBox("Shop Features", "Auto sell, buy items, weather control", "🛒", 3)
createInfoBox("Webhook Logger", "Discord notifications for caught fish", "🔗", 4)
createInfoBox("Camera Controls", "Unlimited zoom & freecam features", "📷", 5)
...
new("UICorner",{Parent=discordBox, CornerRadius=UDim.new(0, 6)})
...
discordButton.MouseButton1Click:Connect(function()
    ...
    copyLabel.TextColor3 = Color3.fromRGB(87, 242, 135)
    task.wait(2)
    copyLabel.Text = "Click to Copy"
...
new("UICorner",{Parent=creditsBox, CornerRadius=UDim.new(0, 6)})

local creditsStroke = new("UIStroke",{
    Parent=creditsBox,
    Color=colors.border,
    Thickness=1,
    Transparency=0.7
...
local creditsTitle = new("TextLabel",{
    Parent=creditsBox,
    Text="Credits",
...
local creditsText = new("TextLabel",{
    Parent=creditsBox,
    Text=[[Created with ♥ by Beee
...
local function adjustForScreenSize()
    ...
    if viewportSize.X < 800 or viewportSize.Y < 600 then
        win.Size = UDim2.new(0, math.min(windowSize.X.Offset, viewportSize.X - 40), 0, math.min(windowSize.Y.Offset, viewportSize.Y - 40))
    else
        win.Size = windowSize
    end
...
if Notify then
    task.wait(0.5)
    Notify.Send("LynX v2.3", "GUI berhasil dimuat!", 4)
end
...
gui:GetPropertyChangedSignal("Parent"):Connect(function()
    if gui.Parent ~= originalParent then
...
gui.Destroying:Connect(function()
    ...
    if blatantv1 then blatantv1.Stop() end
    if blatantv2fix then blatantv2fix.Stop() end
    if UltraBlatant then UltraBlatant.Stop() end
    if blatantv2 then blatantv2.Stop() end
    if NoFishingAnimation then NoFishingAnimation.Stop() end
    if LockPosition then LockPosition.Stop() end
    if DisableCutscenes then DisableCutscenes.Stop() end
    if WalkOnWater then WalkOnWater.Stop() end
    if AutoFavorite then AutoFavorite.Stop() end
    if AutoSellSystem then 
        if AutoSellSystem.Timer then AutoSellSystem.Timer.Stop() end
        if AutoSellSystem.Count then AutoSellSystem.Count.Stop() end
    end
    if AutoBuyWeather then AutoBuyWeather.Stop() end
    if WebhookModule then WebhookModule:Stop() end
    if FreecamModule then 
        if FreecamModule.Stop then FreecamModule.Stop() end
        if FreecamModule.EnableF3Keybind then FreecamModule.EnableF3Keybind(false) end
    end
    if UnlimitedZoomModule then 
        if UnlimitedZoomModule.Disable then UnlimitedZoomModule.Disable() end
    end
    if AntiAFK then AntiAFK.Stop() end
    if MovementModule then 
        if MovementModule.DisableSprint then MovementModule.DisableSprint() end
        if MovementModule.DisableInfiniteJump then MovementModule.DisableInfiniteJump() end
    end
    if FPSBooster then FPSBooster.Disable() end
    if DisableRendering then DisableRendering.Stop() end
    if HideStats then HideStats.Disable() end
    if PingFPSMonitor then PingFPSMonitor:Hide() end
    if GoodPerfectionStable then GoodPerfectionStable.Stop() end
    if SkinAnimation then SkinAnimation.Disable() end
    if AutoTotem3X then AutoTotem3X.Stop() end
    if EventTeleport then EventTeleport.Stop() end
...
local function exportConfig()
    local success, config = ConfigSystem.Load()
    if success then
        local jsonData = HttpService:JSONEncode(config)
        setclipboard(jsonData)
        if Notify then
            Notify.Send("Export Config", "Configuration copied to clipboard!", 3)
        end
        return true
    end
    return false
...
local function importConfig()
    local success, err = pcall(function()
        ...
        if clipboardData and clipboardData ~= "" then
            local decodedConfig = HttpService:JSONDecode(clipboardData)
            if type(decodedConfig) == "table" then
                CurrentConfig = decodedConfig
                ConfigSystem.Save()
                if Notify then
                    Notify.Send("Import Config", "Configuration imported successfully!", 3)
                end
                return true
            end
        end
    ...
    if not success then
        if Notify then
            Notify.Send("Import Failed", "Invalid configuration data in clipboard!", 3)
        end
    end
    return success
end
...
_G.LynxGUI = {
    ...
    Minimize = function()
        if not minimized then
            btnMinHeader.MouseButton1Click:Fire()
        end
    end,
    Maximize = function()
        if minimized and icon then
            icon.InputEnded:Fire({UserInputType = Enum.UserInputType.MouseButton1})
        end
    end,
    Toggle = function()
