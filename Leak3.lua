...
makeToggle(catAutoFav, "Enable Auto Favorite", "AutoFavorite.Enabled", function(on)
    ...
    if AutoFavorite then
        if on then
            ...
            -- Read directly from config to ensure values are available on startup
            ...
            if #tiersToUse > 0 then AutoFavorite.EnableTiers(tiersToUse) end
            if #variantsToUse > 0 then AutoFavorite.EnableVariants(variantsToUse) end
            AutoFavorite.Start()
        else
            AutoFavorite.Stop()
            AutoFavorite.ClearTiers()
